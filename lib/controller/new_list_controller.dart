  ...
  Future<void> addNewList() async {
    ...
    await listData.createNewList(
      ShoppingListModel(
        ...
        time: time.value.isNotEmpty
            ? DateTime.parse("0000-00-00 ${time.value}")
  ...
  Future<void> shareQrCode({required GlobalKey qrKey}) async {
    try {
      ...
      await SharePlus.instance.share(
        ShareParams(text: 'Listaa QR Code', files: [XFile(file.path)]),
      );
    } catch (e) {
      // print(e);
  ...
  DateTime getScheduledDate() {
    return DateTime(
      date.value.isNotEmpty
          ? DateTime.parse(date.value).year
          : DateTime.now().year,
      date.value.isNotEmpty
          ? DateTime.parse(date.value).month
          : DateTime.now().month,
      date.value.isNotEmpty
          ? DateTime.parse(date.value).day
          : DateTime.now().day,
      time.value.isNotEmpty
          ? DateTime.parse("0000-00-00 ${time.value}").hour
          : DateTime.now().add(Duration(hours: 12)).hour,
      time.value.isNotEmpty
          ? DateTime.parse("0000-00-00 ${time.value}").minute
          : DateTime.now().add(Duration(hours: 12)).minute,
    );
  }
